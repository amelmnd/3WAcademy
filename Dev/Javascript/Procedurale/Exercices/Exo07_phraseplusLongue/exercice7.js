...
var string = phrases[0].length;
//declaration du premiere elemnt de commpraison
for (var i = 0; i < phrases.length; i++){
    //boucle for pour parcourir le tableau
    str = phrases[i].length;
    //variable pour optenir la longuer de chaque element
    if (str > string) {
        //declaration de la condition
        stringLengh = phrases[i];
        //enregistrement de la variable la plus longue a un index i 
        string = stringLengh.length;
        //enregistrement de la longeur de cette index 
    }
}
document.write("La phrase la plus longue  est " + stringLengh + "<br>" + "Sa longueur est de " + string + "<br>");
//restranscription de la phrase la plus longue


//Réussit à trouver un bout seul puis avec de l'aide du prof mais Solution final réussit sans la correstion du prof (qusiment unique dans la classe)
...
for (var i = 0; i < phrases.length; i++) {
    if (phrases[indexLongestPhrase].length < phrases[i].length) {
